    ...
    function login(username, password) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (role == 2) {
                    loadMainCompetitionLS(data.adminId, token);
                    console.log("ok");
                }
                if (role == 1) {
                    console.log("pase x el rol == 1" + data.adminId);
                    loadDelegationLS(data.adminId, token); 
                }
...
function loadMainCompetitionLS(idAdmin, tokenAdmin) {
...
function loadDelegationLS(idAdmin, tokenAdmin) {
  console.log(WS_URLS.ADMIN_DELEGATION_LISTAR_DESDE_HASTA + idAdmin);
        $.ajax({
        type: "GET",
        dataType: "json",
        url: WS_URLS.ADMIN_DELEGATION_LISTAR_DESDE_HASTA + idAdmin,
        cache: false,
        success: function (data) {
            console.log(data);
            localStorage.setItem("delegationId", data.delegationId.delegationId);
            localStorage.setItem("delegationName", data.delegationId.name);
        }
    });
};
